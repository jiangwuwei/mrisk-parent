...
<head>
    ...
    <script type="text/javascript" src="<c:url value='/src/js/app/page/policyCenter.js'/>"></script>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#search_btn").on("click", function () {
                window.location = '<c:url value="/policy/toEdit.do?sceneNo=${policy.sceneNo}&policyId="/>' + $("#policy_id").val() + '&ruleName=' + $("#ruleName").val() + '&editTab=1';
            });

            $("#close_policy_btn").on("click", function () {
                window.location = '<c:url value="/policies/list.do?sceneNo=all"/>';
            });
            ...
            $("#save_policy_btn").on("click", function () {
                ...
                var arr = minMax.split(",");
                $(formId + " input[name='min']").val(arr[0]);
                $(formId + " input[name='max']").val(arr[1]);
                submitAjaxForm2(formId, "<c:url value='/policy/insertOrUpdate.do'/>", function (data) {
                    window.location = "<c:url value='/policy/toEdit.do?sceneNo='/>" + data.sceneNo + '&policyId=' + data.policyId;
                });
            });
