...
//                $(this).find(".iconfont").html("&#xe612");
            ...
            $("#search").on("click", function () {
                window.location = '${basePath}/policies/list.do?policiesName=' + $("#policiesName").val();
            });

            $("#selectSceneNo").on("change", function () {
                window.location = '${basePath}/policies/list.do?sceneNo=' + $("#selectSceneNo").val();
            });
            ...
            var confirm_del_policies = new Confirm({
                ...
                confirmHandel: function ($theBtn) {
                    delBySceneNo($theBtn.closest('.list-celue').attr('sceneNo'), '${basePath}/policies/delById.do');
                }
            });
        ...
        function toEditPolicy(sceneNo, policyId) {
            window.location = '${basePath}/policy/toEdit.do?sceneNo=' + sceneNo + '&policyId=' + policyId;
        }
        function toEditPolicyRouter(sceneNo) {
            window.location = '${basePath}/policyRouter/toEdit.do?sceneNo=' + sceneNo;
        }
    </script>
</head>
<body>
<div class="content">
    <div class="search clearfix">
        <input type="hidden" id="updateUrl" value="${basePath}/policies/update.do">
        <input type="hidden" id="insertUrl" value="${basePath}/policies/insert.do">
        <input type="hidden" id="updateStatusUrl" value="${basePath}/policy/updateStatus.do">
    ...
    <div class="list mar20 tablebox">
        <c:forEach var="pair" items="${list}" varStatus="status">
            <div class="list-celue" sceneNo="${pair.key.sceneNo}" policiesName="${pair.key.name}"
                 description="${pair.key.description}"
                 createdDateStr="${pair.key.createdDateStr}">
                        ...
                        <li class='clearfix'>
                            <p class='list-icons'>
                                <i class='boxs rule-run-state' updateId="${policy.id}" status="${policy.status}">
                                    <span class='${policy.status eq 1?"selected":"closed"} to_update_status'
                                          toStatus="1">关闭</span>
                                    <span class="${policy.status eq 2?"selected":"closed"} to_update_status"
                                          toStatus="2">正式</span>
                            ...
                            <h2>
                                <i class="dot"></i><span>${policy.name} <span style="color:blue">(${policy.policyNo})</span></span>
                            </h2>
                        </li>
                    </c:forEach>
                </ul>
            ...
            <div class="pop-cnt">
                <table cellpadding='0' cellspacing='0' class='table-inside'>
                    <tr>
                        <td>策略集名称</td>
                        <td>
                            <div class="input-text" style='width:300px;'><input name="name" type="text"
                                                                                class="input name input-policy"></div>
                        </td>
                    </tr>
                    <tr>
                        <td>场景标识</td>
                    ...
                    <tr>
                        <td>场景描述</td>
                        <td>
                            <div class="input-text" style='width:300px;'><input name="description" type="text"
                                                                                class="input description input-policy">
                            </div>
                        </td>
                    </tr>
                    <tr>
